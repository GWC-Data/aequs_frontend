...
const ORTLabPage = () => {
  ...
  const [tableData, setTableData] = useState<TableRow[]>([
    {
      ...
      detailsBox: {
        totalQuantity: 20,
        ticketCodeRaised: "TKT-2024-001",
        dateShiftTime: "2024-12-01, Morning Shift",
        project: "Project Alpha",
        batch: "BATCH-001",
        color: "Blue",
        assemblyOQCAno: "ASM-001/OQC-001/ANO-001",
    ...
    },
    {
      ...
      detailsBox: {
        totalQuantity: 35,
        ticketCodeRaised: "TKT-2024-002",
        dateShiftTime: "2024-12-01, Afternoon Shift",
        project: "Project Beta",
        batch: "BATCH-002",
        color: "Red",
        assemblyOQCAno: "ASM-002/OQC-002/ANO-002",
  ...
  useEffect(() => {
    const savedData = localStorage.getItem("stage1TableData");
    if (savedData) {
      setTableData(JSON.parse(savedData));
    }
  }, []);

  // Get current shift time based on current hour
  ...
  // Handle inventory remarks change
  const handleInventoryRemarksChange = (id: number, value: string) => {
    setTableData(prevData =>
      prevData.map(row =>
        row.id === id ? { ...row, inventoryRemarks: value } : row
      )
    );
  };
  ...
  const handleSave = () => {
    // Validate that rows with "Yes" have inventory remarks filled
    ...
    // Enable Stage 2 for rows with received = "Yes" and inventory remarks filled
  ...
  const handleStage2Navigate = (row: TableRow) => {
    ...
    // Navigate to stage2 with row data
                    ...
                    <TableCell className="border">
                      <div className="space-y-1 text-xs bg-gray-50 p-3 rounded">
                        ...
                        <div className="flex gap-2">
                          ...
                          <span className="font-medium">{row.detailsBox.project}, {row.detailsBox.batch}, {row.detailsBox.color}</span>
                        </div>
                        <div className="flex gap-2">
                          <span className="font-semibold text-gray-600">• Assembly/OQC/Ano, reason:</span>
                          <span className="font-medium">{row.detailsBox.assemblyOQCAno}, {row.detailsBox.reason}</span>
                        </div>
                      </div>
                    </TableCell>
