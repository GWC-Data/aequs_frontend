...
import { 
  ...
  Trash2, ArrowLeft, Edit2, Save, X, Upload, Send, ExternalLink
} from 'lucide-react';
...
interface TempScannedPart {
  partNumber: string;
  serialNumber: string;
  scanStatus: 'OK' | 'Cosmetic' | 'Not OK' | null;
}
...
const OQCSystem = () => {
  ...
  const handleStatusChange = (index: number, status: 'OK' | 'Cosmetic' | 'Not OK') => {
  ...
  const handleSaveSession = () => {
    ...
    const newSession: ScanSession = {
      ...
      submitted: false, // NOT submitted yet - will be submitted separately
      sentToORT: false
    };
    ...
    const updatedRecords = testRecords.map(record => {
      if (record.id === selectedTicket.id) {
        ...
        return {
          ...
          status: 'Scanning Complete' // Change status to indicate sessions are saved but not submitted
        };
      }
      return record;
    });
    ...
    setTestRecords(updatedRecords);
    showNotification('success', `Session ${sessionNumber} saved with ${tempScannedParts.length} parts! Please submit the session in Tickets tab.`);
  ...
  // NEW: Function to submit a specific session
...
const handleSubmitSession = (ticketId: number, sessionId: string) => {
  ...
  const getStatusIcon = (status: 'OK' | 'Cosmetic' | 'Not OK' | null) => {
    switch (status) {
  ...
  const getStatusColor = (status: 'OK' | 'Cosmetic' | 'Not OK' | null) => {
    switch (status) {
                  ...
                  <div key={`temp-${index}`} className="p-4 border border-gray-200 rounded-lg">
                    ...
                    <div className="space-y-2">
                      ...
                      <RadioGroup
                        ...
                        onValueChange={(value: 'OK' | 'Cosmetic' | 'Not OK') => 
                      ...
                      >
                        ...
                        <div className="flex items-center space-x-2">
                          <RadioGroupItem value="Cosmetic" id={`cosmetic-${index}`} />
                          <Label htmlFor={`cosmetic-${index}`} className="flex items-center gap-2 cursor-pointer">
