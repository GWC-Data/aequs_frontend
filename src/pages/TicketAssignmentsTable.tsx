...
interface EditableLocationProps {
  value: string;
  assignmentId: string;
  onUpdate: (assignmentId: string, newLocation: string) => void;
}

const EditableLocation: React.FC<EditableLocationProps> = ({ value, assignmentId, onUpdate }) => {
  const [isEditing, setIsEditing] = useState(false);
  const [location, setLocation] = useState(value);

  const handleSave = () => {
    if (location.trim() !== value) {
      onUpdate(assignmentId, location.trim() || "home");
    }
    setIsEditing(false);
  };
    ...
    <div className="flex items-center gap-2">
      {isEditing ? (
        ...
        >
          ...
          <span className="text-gray-400 text-xs">(edit location)</span>
        </div>
      )}
    </div>
  );
...
const TicketAssignmentsTable: React.FC = () => {
  ...
  const loadAllAssignments = () => {
    try {
      ...
      let assignments: StoredBarcodeData[] = stored ? JSON.parse(stored) : [];
  ...
  const handleLocationUpdate = (assignmentId: string, newLocation: string) => {
    try {
      const stored = localStorage.getItem(storageKey);
      if (!stored) return;
      ...
      toast({
        ...
        description: `Location changed to "${newLocation}"`,
        duration: 3000,
      });
  ...
  const filteredSummaries = ticketSummaries.filter(summary => {
    const matchesSearch = summary.ticketCode.toLowerCase().includes(searchTerm.toLowerCase()) ||
      summary.assignments.some(assignment =>
        ...
        assignment.location.toLowerCase().includes(searchTerm.toLowerCase()) // Added location to search
      );
        ...
        <CardContent className="p-6">
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1">
              ...
              <Input
                id="search"
                placeholder="Search by ticket code, serial, part number, or location..."
                value={searchTerm}
                  ...
                  filteredSummaries.flatMap((ticket) => {
                    ...
                    rows.push(
                      <TableRow key={`ticket-${ticket.ticketCode}`} className="bg-gray-50 hover:bg-gray-100">
                        ...
                        <TableCell>
                          <div className={`font-semibold ${ticket.remainingParts === 0
                            ? 'text-green-600'
                            : 'text-orange-600'
                            }`}>
                        ...
                        {/* <TableCell>
                          <div className="text-sm font-medium">
                            {ticket.assignments.length > 0 ? ticket.assignments[0].location : "home"}
                          </div>
                        </TableCell> */}
                        <TableCell>
                          <div className="flex items-center gap-2">
                            <div className={`h-2 w-2 rounded-full ${ticket.remainingParts === 0
                              ? 'bg-green-500'
                        ...
                        <TableCell>
                          <div className="flex gap-2">
                            ...
                            {ticket.remainingParts > 0 && (
                              ...
                              //   Scan
                              // </Button>
                              // In the Ticket Header Row section:
                            <Button
                              variant="default"
                              size="sm"
                              onClick={() => handleScanClick(ticket.ticketCode)}
                      ...
                      ticket.assignments.forEach((assignment, assignmentIndex) => {
                        ...
                        rows.push(
                          ...
                          >
                            ...
                            <TableCell>
                              <EditableLocation
                                value={assignment.location || "home"}
